...
class Merlin {
	...
	function __construct( $config = array(), $strings = array() ) {
		...
		add_action( 'activated_plugin', array( $this, 'switch_theme' ) );
	...
	function required_classes() {
		...
		if ( ! class_exists( 'Merlin_WXR_Parser' ) ) {
			require  REALEST_DIR . 'libraries/merlin/includes/class-merlin-xml-parser.php';
		}

		if ( ! class_exists( 'Merlin_Importer' ) ) {
			require REALEST_DIR . 'libraries/merlin/includes/class-merlin-importer.php';
		...
		if ( ! class_exists( 'Merlin_Helper' ) and  true == $this->help_mode ) {
			require REALEST_DIR . 'libraries/merlin/includes/class-merlin-helper.php';
	...
	function admin_page() {
		...
		// Enqueue styles.
		wp_enqueue_style( 'merlin', plugin_dir_url( __FILE__ ) . '/assets/css/merlin' . $suffix . '.css' , array( 'wp-admin' ), MERLIN_VERSION );

		// Enqueue javascript.
		wp_enqueue_script( 'merlin', plugin_dir_url( __FILE__ ) . '/assets/js/merlin' . $suffix . '.js' , array( 'jquery-core' ), MERLIN_VERSION );
	...
	protected function get_base_content() {
		...
		$base_dir = REALEST_DIR . 'libraries/merlin/demo/';
		...
		if ( ! empty( $content ) && is_array( $content ) ) {
			foreach ( $content as $slug => $data ) {
				...
				$content[ $slug ]['description'] = esc_html__( 'Sample', 'merlin-wp' ) . ' ' . $slug . ' ' . esc_html__( 'data.', 'merlin-wp' );
